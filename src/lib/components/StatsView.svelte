<script lang="ts">
	...
	$: availableTickets = $selectedEventletCombo?.ticket_limit ? $selectedEventletCombo?.ticket_limit - $eventletAttendees?.length : null;

	$: checkinChartData = {
		labels: ['Checked in', 'Not Checked in', availableTickets ? 'Available' : null].filter((e)=>e),
		datasets: [
			{
				label: 'Registrations',
...
<div class="stats-container" bind:clientWidth={chartWidth}>
